...
#		endif // ~ debug builds
	...
	SwapChanges recreate_swapchain( VulkanWindow& aWindow )
	{
		...
		try
		{
			std::tie(aWindow.swapchain, aWindow.swapchainFormat,
				aWindow.swapchainExtent)
				=
				create_swapchain(aWindow.physicalDevice, aWindow.surface, aWindow.device,
					aWindow.window, queueFamiliesIndices, oldSwapchain);
		}
		catch (...)
		{
		...
		create_swapchain_image_views(aWindow.device, aWindow.swapchainFormat,
			aWindow.swapImages, aWindow.swapViews);

		// Determine which swap chain properties have changed and return the information
		// indicating this
		SwapChanges ret{};
		if (oldExtent.width != aWindow.swapchainExtent.width ||
			oldExtent.height != aWindow.swapchainExtent.height)
		{
...
namespace
{
	...
	std::tuple<VkSwapchainKHR,VkFormat,VkExtent2D> create_swapchain( VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface, VkDevice aDevice, GLFWwindow* aWindow, std::vector<std::uint32_t> const& aQueueFamilyIndices, VkSwapchainKHR aOldSwapchain )
	{
		...
		// Prefer FIFO_RELAED if it's available
		...
		{
			// Multiple queue may access this resources.
		...
		if (auto const res = vkCreateSwapchainKHR(aDevice, &chainInfo, nullptr,
			&chain); VK_SUCCESS != res)
		{
	...
	void get_swapchain_images( VkDevice aDevice, VkSwapchainKHR aSwapchain, std::vector<VkImage>& aImages )
	{
		...
		if (auto const res = vkGetSwapchainImagesKHR(aDevice,
			aSwapchain, &numSwapchainImage, nullptr); VK_SUCCESS != res)
		{
			throw lut::Error("Error: Unable to enumerate swapchain images\n"
				"vkGetSwapchainImagesKHR() returned %s", lut::to_string(res).c_str());
		}

		//std::vector<VkImage> swapchainImage(numSwapchainImage);
		aImages.resize(numSwapchainImage);
		if (auto const res = vkGetSwapchainImagesKHR(aDevice,
			aSwapchain, &numSwapchainImage, aImages.data()); VK_SUCCESS != res)
		{
	...
	void create_swapchain_image_views( VkDevice aDevice, VkFormat aSwapchainFormat, std::vector<VkImage> const& aImages, std::vector<VkImageView>& aViews )
	{
		...
		for (std::size_t i = 0; i < aImages.size(); ++i)
		{
			...
			if (auto const res = vkCreateImageView(aDevice, &viewInfo, nullptr,
				&view); VK_SUCCESS != res)
			{
	...
	std::optional<std::uint32_t> find_queue_family( VkPhysicalDevice aPhysicalDev, VkQueueFlags aQueueFlags, VkSurfaceKHR aSurface )
	{
		...
		for (std::uint32_t i = 0; i < numQueues; ++i)
		{
			...
			if (aQueueFlags == (aQueueFlags & family.queueFlags))
			{
				...
				auto const res = vkGetPhysicalDeviceSurfaceSupportKHR(aPhysicalDev,
					i, aSurface, &supported);
...
namespace
{
	float score_device( VkPhysicalDevice aPhysicalDev, VkSurfaceKHR aSurface )
	{
		...
		{
			std::fprintf(stderr, "Info: Discarding device '%s': can't present to "
				"surface\n", props.deviceName);
			return -1.0f;
		}
		...
		{
			std::fprintf(stderr, "Info: Discarding device '%s': no graphics queue"
				"family\n", props.deviceName);
			return -1.0f;
		}
